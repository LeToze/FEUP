...
int setFile() {
  ...
  //scanf("%s", c1.filename);
	strcpy(c1.filename,"pinguim.gif");
  if(c1.filename == NULL) {
...
int sendData() {
  ...
  if (sendControlPacket(CONTROLSTART) < 0) {
   ...
   printf("START Controlpacket sent\n");
  ...
  while ((bytesRead = read(applay.fileDescriptor, buffer, applay.fragmentSize)) > 0) {
    ...
    seqNumber++;
	printf("Sequence Number: %d",seqNumber);
    seqNumber %= 255;
  }
...
for(int j=0; j < startpackage_len;j++){
	fflush(stdout);
	printf("CONTROLpacket: %x\n",startpackage[j]);
	fflush(stdout);
}
	fflush(stdout);
	printf("Done Writing\n");
...
int receiveData() {
  ...
  unsigned char *buffer=malloc(applay.fragmentSize + 1);;
