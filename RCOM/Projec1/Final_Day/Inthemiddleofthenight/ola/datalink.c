...
unsigned char *Destuffing (unsigned char *frame,int length){
	//int length = strlen(frame);
	unsigned char *stuffedFrame=malloc(sizeof(unsigned char)*length+1);
  //unsigned char stuffedFrame[length];
	int i,j=0,z=0,g;
  unsigned char bcc;

/*  for(z=0;z < length ;z++){
    stuffedFrame[z] =stuffedFrame[z+4];
}*/
printf("FRAME:%x",frame[0]);

	for(i=0;i < length-2-4 ;i++,j++){
		if(frame[i] == ESC_BYTE){
			stuffedFrame[j] = frame[i+1]^0x20;
			i++;
		}
		else{
  ...
  if(bcc == 0){
    fflush(stdout);
    printf("Destuffing Sucedido\n");
    fflush(stdout);
  }
  fflush(stdout);
  printf("VALOR DO Stuffed :%x",stuffedFrame[0]);
  fflush(stdout);
  printf("VALOR DO Stuffed :%x",stuffedFrame[1]);
  fflush(stdout);
  printf("VALOR DO BCC:%x",bcc);
  fflush(stdout);
  fflush(stdin);
  //sleep(1);
...
int llread(int fd, unsigned char *buffer,int length,int mode){
...
//printf("Valor do i:%d\n",strlen(buffer));
//strcpy(buffer,Destuffing(buffer,length));
//printf("YTAWDFAWED %d\n",i);fflush(stdout);
for(int y=0;y < i;y++){
  fflush(stdout);
	printf("Stufed:%x\t",buffer[y]);
  fflush(stdout);
	//buffer[y+4]=0;
}
