...
int PingOP(){
	printf("c1.filename:%s",c1.filename);
...
int sendData() {
  ...
  int bytesRead = 0, seqNumber = 1000;
...
int i=0;
  ...
  int bytesRead = 0, seqNumber = 0, counter = 3;
  unsigned char buffer[255];
	unsigned char inutil[20];
  while(counter < c1.filesize) {
    ...
    //free(buffer);
		...
		llread(applay.fd ,inutil,3,RECEIVER);
...
int receiveControlPacket() {
	...
	unsigned int package_size = llread(applay.fd ,(read_package),25,RECEIVER);
	fflush(stdout);
	...
	printf("\nfileNAME %s    fileSIZE%d\n",c1.filename,n_bytes );fflush(stdout);
			break;
...
int receivePacket(unsigned char* buffer, int seqNumber) {
	...
	if (llread(applay.fd,information,255,RECEIVER) < 0) {
  ...
  printf("Pacote nº: %d recebido\n",count);
  fflush(stdout);

  if(count ==44){
    if (llread(applay.fd,information,3,RECEIVER) < 0) {
  		printf("error in llread\n");
  		return -1;
  	}
    fflush(stdout);
