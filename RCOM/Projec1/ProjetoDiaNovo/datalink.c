...
void maq_estados(char c,unsigned char sign, int mode){
...
int maq_estados_I(char c, int mode) {
	printf(" Recebi o caracter %x  no estado %d \n", c, estado);
	int fseq;
	char ESC_BYTE = 0x7d;
	char buffer[255];
...
/*while (REJDONE == FALSE) {      
	...
	fseq=0;
	if(fseq){
	maq_estados(buf1[0], REJ1,RECEIVER);
	}
	if(!fseq){
	maq_estados(buf1[0], REJ0,RECEIVER);
	}
} */
return 0;
}
